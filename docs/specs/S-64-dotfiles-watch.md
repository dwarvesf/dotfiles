...
- [x] `tests/dotfiles-watch.sh` passes (10/10 on Mac mini after the `^.M ` regex fix)
- [x] User-facing section in `docs/guide.md` explaining install / status / tail
- [x] End-to-end demo on Mac mini 2026-05-12: appended a comment to `~/.tool-versions`, manual `dotfiles-watcher-tick` (and one of several launchd fires) wrote `TICK start` + `+ .tool-versions` + `TICK done` to `~/Library/Logs/dotfiles-watcher.log`; `git diff home/dot_tool-versions` in the dotfile repo showed the line absorbed; sed-revert + re-fire returned source to HEAD.

## Known issues

- **`MM` rows that never clear.** `.claude/settings.json` is generated by a chezmoi `modify_` script that yields a different output than the live file. `chezmoi re-add` succeeds but doesn't change source content. Each tick logs an absorb attempt that's effectively a no-op. The `DRIFT_LOOP_MAX=3` cap keeps it bounded. Long-term fix: detect "re-add was a no-op" and stop retrying for that file in the same tick. Tracked as a follow-up.
- **launchd `runs` outpaces log lines.** Manual `dotfiles-watcher-tick` invocations hold the `mkdir` lock for ~6s (debounce + drift loop), during which launchd-triggered firings exit silently via the `mkdir`-fails-→-exit path. The lock collapses bursts correctly; the only observable effect is "runs counter increments faster than log entries." Not a correctness issue; documented here so it doesn't get re-investigated as a bug.
