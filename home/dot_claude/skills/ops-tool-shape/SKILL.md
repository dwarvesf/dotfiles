...
name: ops-tool-shape
description: |
  Verify or set up the file and folder structure of a tool under `~/workspace/tieubao/ops-toolkit/tools/`. Use when scaffolding a brand-new tool, restructuring an existing tool to match the standard, or auditing which doc files a tool currently has vs needs. Symptoms include "scaffold a new ops-toolkit tool", "audit <tool>'s structure", "what files does <tool> need?", "is <tool>'s layout compliant?", "set up the folder for a new tool", "what's missing from <tool>?", "the housekeeper structure looks great, what shape should <X> be?". Outputs a tool directory with the right folder layout (empty stubs only; no doc content), plus a row update in `ops-toolkit/_meta/INVENTORY.md`. After this skill finishes, the sibling skill `ops-tool-docs` fills in the doc content. Canonical reference (read it, don't copy it): `~/workspace/tieubao/ops-toolkit/tools/housekeeper/`. NOT for writing doc CONTENT (that's `ops-tool-docs`), NOT for tools outside ops-toolkit, NOT for `experiments/<slug>/` (those follow a different gradient), NOT for third-party-tool deploy bundles where upstream owns the docs.
---
...
The canonical reference instance is `~/workspace/tieubao/ops-toolkit/tools/housekeeper/`. Read it for the actual shape; do not copy filenames out of it.
...
- "the housekeeper folder shape looks right, apply that to X"
...
|---|---|---|
| Tool name | `housekeeper` (kebab-case) | scaffold |
...
| `daemon-service` (LaunchAgent / long-running process, like `housekeeper`, `notion-export`) | ✓ | ✓ | ✓ | ✓ | opt-until-multi | opt-until-multi | opt-until-multi | ✓ |
...
| `mixed` (rare; multi-language tool with substantial structure, like `hermes`, `vps-mon`; **also the canonical reference `housekeeper`** because it grew daemon + CLI + multi-spec roadmap) | ✓ | ✓ | ✓ | ✓ | opt-until-multi | ✓ | opt-until-multi | ✓ |
...
5. **Add `.gitignore`** scoped to the tool (Python builds + the SQLite/log artifacts the tool will produce). Steal from `tools/housekeeper/.gitignore` if Python.
...
- **Leaving per-feature SPECs at tool root.** A `SPEC-<feature>.md` file at the tool root is wrong even if there is only one of them. Tool-root `SPEC.md` is the **index** only. Per-feature behaviour contracts always live under `docs/specs/`. Housekeeper ADR-0002's "OK for ≤3 specs" line described a build-time observation for `mac-backup`'s pre-standard layout; it is not a general accepted-deviation. New tools and existing tools being touched should use `docs/specs/` regardless of feature count. (If a tool's per-feature spec sits at root, move it: `git mv tools/<x>/SPEC-foo.md tools/<x>/docs/specs/SPEC-foo.md` and update cross-references.)
...
- **Modifying the canonical reference.** `tools/housekeeper/` is the live calibration instance. Do not edit it as part of scaffold or audit.

## Worked example: housekeeper

This is the shape this skill teaches. Read each file for what it accomplishes:

| Path | What this file is |
|---|---|
| `README.md` | One-page intro: what is housekeeper, install command, links to MANUAL/RUNBOOK/SPEC. |
| `MANUAL.md` | 14-section task guide ("how do I undo a move?", "how do I disable Tier B?"). |
| `RUNBOOK.md` | Incident manual (TCC blockers, daemon crash-loops, sensitive false positives). |
| `SPEC.md` | Index: a table linking the other docs + multi-feature roadmap. |
| `docs/architecture.md` | Six ASCII diagrams + reader-takeaway captions. |
| `docs/specs/SPEC-001-sort.md` | The active feature's behaviour contract (schema, gates, code paths). |
| `docs/specs/SPEC-002-archive.md` etc. | Placeholder specs for future features. |
| `docs/decisions/0001-three-tier-classifier.md` ... `0005-...md` | ADRs sequenced by build order; explain WHY. |
| `tool.toml` | ops-toolkit registration with consumers, systems, secrets. |
| `pyproject.toml` | uv-managed Python. |
| `deploy/macos/mini.housekeeper.plist` | LaunchAgent definition. |
